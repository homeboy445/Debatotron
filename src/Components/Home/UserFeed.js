...
const UserFeed = () => {
  ...
  const LikePost = (index, typeoflike) => {
    ...
    const postOrDebateId =
      currentFeed[index].type === CONTENT_TYPE.DEBATE
        ? currentFeed[index].debate.id
        : currentFeed[index].post.id;
    const postOrDebateType =
      currentFeed[index].type === CONTENT_TYPE.DEBATE ? "debate" : "post";
      ...
      const fetchUserFeed = async () => {
        try {
          const feedResponse = await axios.get(
            `${Main.serverURL}/feed/${Main.userInfo[0].id}`,
            Main.getAuthHeader()
          );
          ...
          const popularUsersResponse = await axios.get(
            `${Main.serverURL}/popularUsers`,
            Main.getAuthHeader()
          );
          updatePopularUsers(popularUsersResponse.data);

          const topContributorsResponse = await axios.get(
            `${Main.serverURL}/topContributors`,
            Main.getAuthHeader()
          );
          updateContributors(topContributorsResponse.data);

          const feedLikesResponse = await axios.get(
            `${Main.serverURL}/getfeedlikes/${Main.userInfo[0].id}`,
            Main.getAuthHeader()
          );
  ...
  return !fetchStatus ? null : (
    <div className="feed">
      <h1 className={`home_title ${postBox ? "opacque" : ""}`}>Debatotron</h1>
